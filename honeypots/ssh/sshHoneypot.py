...
def handle_client(client_socket, client_ip):
    ...
    try:
        logger.info(f"Starting IP enrichment for {client_ip}")
        enrich_ip(client_ip)
        logger.info(f"Finished IP enrichment for {client_ip}")
    except Exception as e:
        logger.error(f"IP enrichment failed for {client_ip}: {e}")
